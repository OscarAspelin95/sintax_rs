...
def parse_tsv(tsv: Path, threshold: float, outdir: Path):
    ...
    for asv, asv_subset in df.groupby(by="asv"):
        ...
        for level in Levels.as_list():
            ...
            best_level = re.sub(r"(d|k|p|c|o|f|g|s):", "", best_level)

            best_score = float(best_score) / len(asv_subset)
            asv_full.append(f"{best_level}({best_score:.4f})")

            if asv not in asv_short and best_score >= threshold:
                asv_short[asv] = SintaxAsvResult(
                    level=level, tax_name=best_level, score=best_score
                )
